#!/usr/bin/env bash
#
# release.sh — drive a single-service continuo release from CD.
#
# This service's manifest is already uploaded to the canonical S3 key
# <service>/<release_id>/manifest.json and its image already pushed by the
# calling workflow. This script crosses into the Hetzner cluster over SSH,
...
#   2. POSTs the single-service release;
#   3. polls GET /releases/<id> to a terminal status, failing the deploy on
#      `rejected`.
#
# One release = one service. The platform reassembles the full topology from
# its service_prod pointer table.
#
...
#   RELEASE_ID       — unique per run, e.g. rel-<shortsha>-<runid>-<service>
#   SERVICE          — the single service being released, e.g. service-1
#   IMAGE_TAG        — the service image tag, e.g. <shortsha>
#
# Optional:
...
echo "Driving release ${RELEASE_ID} (service=${SERVICE}) against ${HETZNER_HOST}"

# The remote script runs on continuo-server. Its positional args ($1..$5) carry
...
BODY=$(printf '{"service":"%s","release_id":"%s","image_tag":"%s","bootstrap":%s}' \
  "$SERVICE" "$RELEASE_ID" "$IMAGE_TAG" "$BOOTSTRAP")
