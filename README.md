...
The integration tests in `dbt-loader/integration/test_upload.py` exercise real dbt compile and S3 uploads and run via the compose stack above (localstack + Postgres + the self-contained dbt-loader image). The CLI, config, compile-wrapper, and per-release upload-layout tests in `dbt-loader/tests/` run without any external services.
